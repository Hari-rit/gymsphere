  ...
  return (
    <div className="d-flex flex-column align-items-center gap-4 w-100">
      {approvedForms.map((form) => {
        const att = attendance?.[form.user_id]?.[today] || null;
                ...
                <div className="d-flex justify-content-center gap-2 flex-wrap">
                  <button
                    className={`btn btn-sm ${
                      att === "present" ? "btn-success" : "btn-outline-success"
                    }`}
                    disabled={!!att}
                    onClick={() => handleAttendance(form.user_id, "present")}
                  >
                    {att === "present" ? "✅ Present" : "Mark Present"}
                  </button>
                  <button
                    className={`btn btn-sm ${
                      att === "absent" ? "btn-danger" : "btn-outline-danger"
                    }`}
                    disabled={!!att}
                    onClick={() => handleAttendance(form.user_id, "absent")}
                  >
                    {att === "absent" ? "❌ Absent" : "Mark Absent"}
                  </button>
                </div>
